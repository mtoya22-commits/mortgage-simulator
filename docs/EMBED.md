...
## 結果画面の下部が切れるとき（トラブルシュート）

アプリは結果画面でも実コンテンツ高さを送り続けます（チャートやフォント確定後も再送）。
下部が切れるのは、**親 iframe が受信した高さに追従できていない**ことがほぼ原因です。次を確認してください。

1. **最新の受信スクリプトが固定ページに入っているか**（`lifeplanlab:resize` を受けて
   `iframe.style.height` を更新する版）。古い版・未設置だと iframe は広がりません。
2. **iframe に固定の `height=` 属性を付けない**。高さはスクリプトが `style.height` で更新します。
   `height="800"` のような固定属性や、`style="height:800px"` を**書かない**でください（`min-height` のみ可）。
3. **親テーマ/ラッパーの CSS** に `max-height` や `overflow:hidden` が無いか。iframe を囲む要素が
   高さを制限していると、iframe を広げても見た目が切れます。
4. `scrolling="no"` のままで OK です（高さが追従していれば内部スクロールは不要）。逆に高さが
   追従していない状態で `scrolling="no"` だと、はみ出し分がそのまま切れます。
5. それでも切れる場合は、ブラウザのコンソールで `window` の `message` を確認し、
   `{type:'lifeplanlab:resize', height: …}` が届いているか／`event.source` 判定で弾いていないかを確認します。
